...
これにはいくつかの題目に沿った枠が用意されており、それを埋めていくことで活用します。1番目に顧客セグメントとアーリーアダプター、2番目に課題と既存の代替品、3番目に独自の価値提案を埋めていきます。この3つはリーンキャンバスの軸となるので、はじめに埋めることを推奨します。まずは誰の何をどんな独自性を持って解決したいのかを明確にし、その後ビジネス面におけるいくつかの事項を考慮に入れます。このとき、ソリューションを何にするかが1つの肝になりますが、ソリューションは〓次節〓で紹介するブレーンライティングを活用してから定めることがおすすめの手段です。

リーンキャンバスについて短く語ることは困難ですが、重要なのは荒くとも全てを一度埋めることです。一度埋めてから何度も見直すことで、アイデアは磨かれていきます。順番に悩むのであれば、表の順番に沿って埋めてみてください。1番目から3番目までは順番通り埋めることを推奨しますが、それ以降は埋めやすいものがあれば、順番に沿う必要は特にありません。埋め切ったら、題目同士の整合性や関係性にも着目しながら見直し、アイデアを検証しましょう。時には、一晩寝かせることで新たな発見があるかもしれません。
